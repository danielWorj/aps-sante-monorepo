...
const ANNOUNCEMENT_FALLBACK_IMAGE = heroBg;

function AnnouncementsBand() {
      ...
      <div className="announcements-track">
        {loopItems.map((item, i) => (
          <Link
            ...
            key={`${item.id}-${i}`}
          >
            <img src={item.file_url || ANNOUNCEMENT_FALLBACK_IMAGE} alt={item.libelle} />
            <div className="announcement-text">
              <span className="announcement-libelle">{item.libelle}</span>
              {item.description && (
                <span className="announcement-description">{item.description}</span>
              )}
            </div>
          </Link>
        ))}
