 ...
 * "Médecins et professionnels", croquis : 1.1 → 1.1.1 ville → 1.1.2
 * spécialité → 1.1.3 liste des médecins obtenus).
 *
 ...
 * correspondantes.
 *
 * pays_exercice_id / ville_exercice_id peuvent arriver en query params
 * (retour arrière depuis l'Écran 1.1.2 via son lien "Retour" — voir
 * note du croquis : "on peut passer de 1.1.2 → 1.1.1") : dans ce cas
 * ils priment sur la détection automatique, pour ne pas faire perdre
 * à l'utilisateur un choix déjà fait.
 */
export default function OnboardingMedecinVille() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();

  const paysInitial = searchParams.get('pays_exercice_id') || '';
  const villeInitiale = searchParams.get('ville_exercice_id') || '';
  ...
  function allerVersEtapeSpecialite() {
    if (!villeId) return;
    ...
    navigate(`/onboarding/medecins/specialite?${params.toString()}`);
  }
    ...
    <section className="onboarding-shell onboarding-shell-top">
      <div className="container-aps onboarding-container-lg">
        ...
        <div className="form-nav-actions">
          ...
          <button type="button" className="btn btn-primary" disabled={!villeId} onClick={allerVersEtapeSpecialite}>
